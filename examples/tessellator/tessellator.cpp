...
OpenSubdivTessellator::OpenSubdivTessellator():
    _scheme(kCatmark),
    _hbrMesh(NULL),
    _farMesh(NULL),
    _ompComputeController(NULL),    
    _cpuComputeController(NULL),
...
bool
OpenSubdivTessellator::Initialize(
    int numVertices,
    ...
    string *errorMessage)
{
    ...
    return Initialize("test", numVertices, levels, nvertsVec, indicesVec,
    ...
    std::string *errorMessage)
{
    ...
    for (int i = 0; i < numVertices; ++i) {
        ...
        if (!hvert) {
            if (errorMessage)
                *errorMessage = "Unable to create OSD vertex from tidscene";
        }
    }
...
//    _hbrMesh->SetInterpolateBoundaryMethod(
//        HbrMesh<OsdVertex>::k_InterpolateBoundaryEdgeOnly);
    
    _hbrMesh->Finish();

    
    // create the quad tables to include all levels by specifying firstLevel as 1
    FarMeshFactory<OsdVertex> meshFactory(_hbrMesh, level, false, /*firstLevel=*/1);

    _farMesh = meshFactory.Create();

    
    _ompComputeController = new OpenSubdiv::OsdOmpComputeController();
    _cpuComputeController = new OpenSubdiv::OsdCpuComputeController();    
...
//    _firstVertexOffset =  parray.GetVertIndex();
...
OpenSubdivTessellator::CreateVertexBuffer() const
{
    return OsdCpuVertexBuffer::Create(3, _farMesh->GetNumVertices());
}

OpenSubdiv::OsdCpuVertexBuffer*
OpenSubdivTessellator::CreateVVBuffer() const
{
    if (_vvNames.empty()) {
        return NULL;
    }
    
    return OsdCpuVertexBuffer::Create(
        _vvNames.size(), _farMesh->GetNumVertices());
}

    
void
OpenSubdivTessellator::SetCoarsePositions(
    ...
    const std::vector<float>& coords) const
{
    ...
    if (numFloats/3 >= _farMesh->GetNumVertices()) {
        std::cout << "Indexing error in tesselator\n";
    } else {
        vertexBuffer->UpdateData(pFloats, 0, numFloats / 3);
    }
}

void
OpenSubdivTessellator::SetCoarseVVData(OpenSubdiv::OsdCpuVertexBuffer *vvBuffer,
                                       const vector<float>& data) const
{
    ...
    int numElements = vvBuffer->GetNumElements();
    int numVertices = (int) data.size() / numElements;
    const float* pFloats = &data.front();
    vvBuffer->UpdateData(pFloats, 0, numVertices);
}
...
void
OpenSubdivTessellator::GetFVData(
...
bool
OpenSubdivTessellator::Refine(OpenSubdiv::OsdCpuVertexBuffer *vertexBuffer,
                              ...
                              string *errorMessage) const
{
    ...
    // XXX:gelder
    // OK to create and delete context each time?  Maybe should be passed in
    // like vertexBuffer
    OsdCpuComputeContext *computeContext =
        OsdCpuComputeContext::Create(_farMesh);

    
    if (useOmp) {
        _ompComputeController->Refine(computeContext,
                                      _farMesh->GetKernelBatches(),
                                      vertexBuffer,        
                                      vvBuffer);
    } else {
        _cpuComputeController->Refine(computeContext,
                                      _farMesh->GetKernelBatches(),
                                      vertexBuffer,        
                                      vvBuffer);        
    }
...
bool
OpenSubdivTessellator::GetPositions(
    OpenSubdiv::OsdCpuVertexBuffer *vertexBuffer,
    vector<float>* coords,
    string *errorMessage) const 
{
    ...
    const float* cBegin = vertexBuffer->BindCpuBuffer();
...
bool
OpenSubdivTessellator::GetQuads(vector<int>* quads,
                                string *errorMessage) const
{
    ...
    for (int i=0; i<_numRefinedQuads*4; ++i) {
...
bool
OpenSubdivTessellator::WriteRefinedObj( const std::string &filename,
                                        std::string *errorMessage)
{
    ...
    OpenSubdiv::OsdCpuVertexBuffer*    vertexBuffer = CreateVertexBuffer();
    OpenSubdiv::OsdCpuVertexBuffer*    vvBuffer = CreateVVBuffer();

    if (not Refine(vertexBuffer, vvBuffer, errorMessage)) {
        delete vertexBuffer;
        delete vvBuffer;                            
        return false;
    }
    ...
    if (not (GetPositions(vertexBuffer, &refinedPositions, errorMessage) and
    ...
    fout << "# Exported by OpenSubdivTessellator\n";
...
// XXX: why does deleting hbrMesh crash?
//    delete _hbrMesh;
    delete _farMesh;
}
...
//           a TessellatorUtils in its regression suite that almost fits the bill.
//
// prideout: 3/19/2013 - edits are not yet supported.
void
OpenSubdivTessellator::_ProcessTagsAndFinishMesh(
...
//------------------------------------------------------------------------------
std::string OpenSubdivTessellator::genRIB() const {
